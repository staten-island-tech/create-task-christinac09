...
function generateCardHTML(card, isDuplicate = false) {
  return `
...
function displayUserStats(user) {
  DOMSelectors.statsBtn.addEventListener("click", function () {
    ...
    user.cards.forEach((card) => {
      cardsContainer.insertAdjacentHTML("beforeend", generateCardHTML(card));
    });
  });
}
...
async function runTriviaRound(user, data) {
  ...
  const correctCharacter = getRandomItem(withDescription);
  const wrongAnswers = [];
  while (wrongAnswers.length < 3) {
    const randomName = getRandomItem(data).name;
    if (
